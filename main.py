...
def get_single_song(bot, update):
    ...
    if config["SPOTDL_DOWNLOADER"]:
        os.system(f'spotdl {url} --st 12 --dt 8 --output-format flac')
    ...
    try:
        ...
        files = [os.path.join(dp, f) for dp, dn, filenames in os.walk(".") for f in filenames if os.path.splitext(f)[1] == '.flac']
        for file in files:
